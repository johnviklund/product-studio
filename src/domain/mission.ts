...
function renderExecuteTaskMd(
  ...
  return [
    ...
    "List every task-owned path that is currently changed in the working tree, even when an earlier attempt made the change. The controller needs those exact paths in changed_files so it can commit the retained implementation.",
    "Report an empty changed_files array only after proving the workspace is clean outside .founder/ and the approved work is already present at the mission source commit; inspecting current file contents alone is not that proof.",
