...
Package hash: 8bb3db375c9eba0b88327ba9f2922afe2bcce819b6f7174018bd7e595827bdaa
...
Write the structured result to `.founder/missions/wi_f2d97c58-451d-43ae-ae32-47b3d9b2137d/execute-2-2-12/result.json`.
...
List every task-owned path whose implementation you inspected and verified, even when an earlier attempt made the change. The controller needs those exact paths in changed_files so it can compare them with the retained worktree and commit the implementation.
Do not run Git merely to discover changed_files; the controller performs that comparison. Report an empty array only when the mission itself establishes that the workspace is clean and the approved work is already present at its source commit; inspecting current file contents alone is not that proof.
Use this complete JSON shape:

```json
{
  "result_schema_version": 2,
  "mission_content_sha256": "8bb3db375c9eba0b88327ba9f2922afe2bcce819b6f7174018bd7e595827bdaa",
  "identity": {
    "phase": "execute",
